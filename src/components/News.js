...
const News = (props) => {
    ...
    const fetchMoreData = async () => {
        const url = `https://newsapi.org/v2/top-headlines?country=${props.country}&category=${props.category}&apiKey=${props.apiKey}&${props.apikey}&${props.apikey}d60b8680fd364a9fb5ae571900162c54&page=${page + 1}&pageSize=${props.pageSize}`;
        setPage(page + 1)
        let data = await fetch(url);
        let parsedData = await data.json()
        console.log(parsedData);
    ...
    return (
        <>
            <h2 className="text-center" style={{ margin: '55px 0px' }}>Daily News - Top {capitalizeFirstLetter(props.category)} HeadLines</h2>
            {loading && <Spiner />}
                    ...
                    <div className="row">
                        {articles.map((element) => {
                            return <div className="col-md-4" key={element.url}>
                                <NewsItem tittle={element.title.slice(0, 40)} description={element.description ? element.description.slice(0, 88) : ""} imgurl={element.urlToImage} nwesurl={element.url} author={!element.author ? "Unkown" : element.author} date={element.publishedAt} source={element.source.name} />
